...
<table class="table--basic"><tr><th>#</th><th>Réf</th><th>Titre</th><th>Prix</th><th>TVA</th><th>Total</th><th>Client</th><th>Modifier</th><th>Supprimer</th></tr>
@foreach($invoices as $invoice)
<tr>
    <td>{{$invoice->id}}</td>
    <td><a href="/invoices/show/{{$invoice->id}}">{{$invoice->ref}}</a></td>
    <td>{{$invoice->title}}</td>
    <td>{{$invoice->price}}€</td>
    <td>{{$invoice->tva}}%</td>
    <td>{{$invoice->total}}€</td>
